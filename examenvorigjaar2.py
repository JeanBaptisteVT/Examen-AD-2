...
class Inventory_Manager:
  ...
  #  def save_to_csv(self, filename):
        # Bestand openen in write-modus
        f = open(filename, "w")

        # Over elk product en elke batch gaan
        for name, product in self.products.items():
            for batch in product.batches:
                # Eén CSV-lijn manueel bouwen
                line = name + "," + str(batch.quantity) + "," + str(batch.cost_per_unit) + "\n"

                # Lijn in bestand zetten
                f.write(line)
    ...
    def load_from_csv(self, filename):
        ...
        # Elke lijn inlezen
        for line in f:
            line = line.strip()  # verwijder \n en spaties
            if line == "":  # lege lijnen overslaan
                continue

            # Splitsen op komma → CSV-velden
            parts = line.split(",")
            product_name = parts[0]
            quantity = int(parts[1])
            cost_per_unit = float(parts[2])

            # Als product nog niet bestaat → nieuw object aanmaken
            if product_name not in self.products:
                self.products[product_name] = Product(product_name, 0, 0)

            # Batch toevoegen aan het juiste product
            self.products[product_name].add_batch(quantity, cost_per_unit)
